...
$site = npx --yes netlify-cli sites:create --name $SiteName 2>&1
Write-Host $site

if (-not (Test-Path ".netlify")) {
    npx --yes netlify-cli init --manual --name $SiteName 2>&1
}
...
Write-Host "  2. GitHub repo ulang (yoki Manual Deploy)"
Write-Host "  3. BOT_TOKEN va ADMIN_IDS ni Environment ga qo'shing"
